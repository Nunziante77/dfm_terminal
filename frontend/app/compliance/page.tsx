...
export default function CompliancePage() {
  ...
  const { data, isFetching, error } = useQuery({
    queryKey: ["compliance", offset, submitted],
    queryFn: () => getCompliance({ entity_id: submitted || undefined, limit: PAGE_SIZE, offset }),
  });
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-terminal-cyan text-sm font-bold tracking-widest">REGULATORY COMPLIANCE</h1>
      ...
      <div className="panel p-3 flex items-center gap-3">
        ...
        <button onClick={() => { setSubmitted(entityId); setOffset(0); }}
          className="text-terminal-cyan text-xs hover:text-white">APPLY</button>
        {submitted && (
      ...
      <div className="flex items-center justify-between">
        <StatusBar loading={isFetching} message={`${total} records · COMPLIANCE`} />
